...
capture log close
log using y97_wages_anyschool_school_interaction_t0_16.log, replace
set matsize 4000
...
sum lnWageNoSelf `Pvars97' `Mvars' `Svars2' `Zvars2' `Evars2' `Fvars' if inlist(activity,2,3,4,12,13,14,22,23,24) & ~mi(lnWageNoSelf), sep(0)
...
ml model lf normal (lnWageNoSelf = `Pvars97' `Mvars' `Svars2' `Zvars2' `Evars2' `risk' `work') /sigma2 /sigma3 /sigma4 if inlist(activity,2,3,4,12,13,14,22,23,24) & ~mi(lnWageNoSelf), vce(cluster id)
ml max
estimates store reg2
qui outreg2 using y97_wages_anyschool_school_interaction_t0_16, excel replace bdec(4) sdec(4) ctitle(WagesDiffVar)
...
outsheet wage_all    using y97_wages_anyschool_school_interaction_t0_16_coef.csv if !mi(wage_all), comma replace
outsheet wage_se_all using y97_wages_anyschool_school_interaction_t0_16_se.csv   if !mi(wage_all), comma replace

putexcel A1=("WageCoefs") A2=matrix(wage_output ,rownames) using y97_wages_anyschool_school_interaction_t0_16.xlsx, sheet ("WageCoefs") modify
putexcel F1=("WageLike")  F2=matrix(like1       ,rownames) using y97_wages_anyschool_school_interaction_t0_16.xlsx, sheet ("WageCoefs") modify
putexcel A1=("WageCoefs") A2=matrix(sigma_output,rownames) using y97_wages_anyschool_school_interaction_t0_16.xlsx, sheet ("WageSigmas") modify
