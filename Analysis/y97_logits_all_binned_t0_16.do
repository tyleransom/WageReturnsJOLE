...
capture log close
log using y97_logits_all_binned_t0_16.log, replace
set matsize 4000
...
use if cohortFlag==1997 & ~female using ${secretdir}/yCombined_scaled_t0_16.dta, clear
...
table activity, c(p5 anySchoolt   p25 anySchoolt   median anySchoolt   p75 anySchoolt   p95 anySchoolt  )
table activity, c(p5 schoolOnlyt  p25 schoolOnlyt  median schoolOnlyt  p75 schoolOnlyt  p95 schoolOnlyt )
table activity, c(p5 workK12t     p25 workK12t     median workK12t     p75 workK12t     p95 workK12t    )
table activity, c(p5 workColleget p25 workColleget median workColleget p75 workColleget p95 workColleget)
table activity, c(p5 workPTonlyt  p25 workPTonlyt  median workPTonlyt  p75 workPTonlyt  p95 workPTonlyt )
table activity, c(p5 workFTonlyt  p25 workFTonlyt  median workFTonlyt  p75 workFTonlyt  p95 workFTonlyt )
table activity, c(p5 militaryt    p25 militaryt    median militaryt    p75 militaryt    p95 militaryt   )
table activity, c(p5 othert       p25 othert       median othert       p75 othert       p95 othert      )

* table activity if inrange(activity,11,17), c(p5 anySchoolt   p25 anySchoolt   median anySchoolt   p75 anySchoolt   p95 anySchoolt  )
* table activity if inrange(activity,11,17), c(p5 schoolOnlyt  p25 schoolOnlyt  median schoolOnlyt  p75 schoolOnlyt  p95 schoolOnlyt )
* table activity if inrange(activity,11,17), c(p5 workK12t     p25 workK12t     median workK12t     p75 workK12t     p95 workK12t    )
* table activity if inrange(activity,11,17), c(p5 workColleget p25 workColleget median workColleget p75 workColleget p95 workColleget)
* table activity if inrange(activity,11,17), c(p5 workPTonlyt  p25 workPTonlyt  median workPTonlyt  p75 workPTonlyt  p95 workPTonlyt )
* table activity if inrange(activity,11,17), c(p5 workFTonlyt  p25 workFTonlyt  median workFTonlyt  p75 workFTonlyt  p95 workFTonlyt )
* table activity if inrange(activity,11,17), c(p5 militaryt    p25 militaryt    median militaryt    p75 militaryt    p95 militaryt   )
* table activity if inrange(activity,11,17), c(p5 othert       p25 othert       median othert       p75 othert       p95 othert      )

* table activity if inrange(activity,21,26), c(p5 anySchoolt   p25 anySchoolt   median anySchoolt   p75 anySchoolt   p95 anySchoolt  )
* table activity if inrange(activity,21,26), c(p5 schoolOnlyt  p25 schoolOnlyt  median schoolOnlyt  p75 schoolOnlyt  p95 schoolOnlyt )
* table activity if inrange(activity,21,26), c(p5 workK12t     p25 workK12t     median workK12t     p75 workK12t     p95 workK12t    )
* table activity if inrange(activity,21,26), c(p5 workColleget p25 workColleget median workColleget p75 workColleget p95 workColleget)
* table activity if inrange(activity,21,26), c(p5 workPTonlyt  p25 workPTonlyt  median workPTonlyt  p75 workPTonlyt  p95 workPTonlyt )
* table activity if inrange(activity,21,26), c(p5 workFTonlyt  p25 workFTonlyt  median workFTonlyt  p75 workFTonlyt  p95 workFTonlyt )
* table activity if inrange(activity,21,26), c(p5 militaryt    p25 militaryt    median militaryt    p75 militaryt    p95 militaryt   )
* table activity if inrange(activity,21,26), c(p5 othert       p25 othert       median othert       p75 othert       p95 othert      )

* Generate some bin variables based on schooling experience
*  Could great as bin variables, and do i.anySchoolBin, but that's not how we roll
* gen   anySchoolt = schoolOnlyt==1 | workK12t==1 | workColleget==1 // in case it's not there
* gen   anySchool1 = anySchoolt>=0 & anySchoolt<1
* gen   anySchool2 = anySchoolt>=1 & anySchoolt<2
* gen   anySchool3 = anySchoolt>=2 & anySchoolt<3
* gen   anySchool4 = anySchoolt>=3 & anySchoolt<4
* gen   anySchool5 = anySchoolt>=4 & anySchoolt<5
* gen   anySchool6 = anySchoolt>=5 & anySchoolt<6
* gen   anySchool7 = anySchoolt>=6 & anySchoolt<7
* gen   anySchool8 = anySchoolt>=7 & anySchoolt<8
* gen   anySchool9 = anySchoolt>=8 & anySchoolt<.
...
* do y97_constraints.do
do y97_constraints_all_binned.do
label val activity .
...
sum lnWageNoSelf `Pvars97' `Mvars' `Svars' `Zvars' `Evars' `Fvars' `Cvars' `Bvars' if inlist(activity,1,2,3,4,5,6,7,11,12,13,14,15,16,17,21,22,23,24,25,26), sep(0)


*============================================================================
* Logits
*============================================================================
mlogit  activity        `Pvars97' `Mvars' `Svars1' `Zvars1' `Evars1' `Fvars' `Cvars' `Bvars1' if inrange(activity, 1, 7), base( 1) difficult constraints(101/202) vce(cluster id)
predict p1risk1 p1risk2 p1risk3 p1risk4 p1risk5 p1risk6 p1risk7 if e(sample), pr
estimates store logit1
qui outreg2 using y97_logits_all_binned_t0_16, excel replace bdec(4) sdec(4)

mlogit  activity        `Pvars97' `Mvars' `Svars2' `Zvars2' `Evars2' `Fvars' `Cvars' `Bvars2' if inrange(activity,11,17), base(11) difficult constraints(221/240) vce(cluster id)
predict p2risk11 p2risk12 p2risk13 p2risk14 p2risk15 p2risk16 p2risk17 if e(sample), pr
estimates store logit2
qui outreg2 using y97_logits_all_binned_t0_16, excel append  bdec(4) sdec(4)

mlogit  activity        `Pvars97' `Mvars' `Svars3' `Zvars3' `Evars3' `Fvars' `Cvars' `Bvars3' if inrange(activity,21,26), base(21) difficult constraints(301/441) vce(cluster id)
...
preserve
	keep logit_coef
	drop if logit_coef==.
	outsheet logit_coef using y97_logits_all_binned_t0_16_coef.csv, comma replace
restore

preserve
	keep logit_se
	drop if logit_se==.
	outsheet logit_se using y97_logits_all_binned_t0_16_se.csv, comma replace
restore
