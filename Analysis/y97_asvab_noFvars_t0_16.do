...
capture log close
log using y97_asvab_noFvars_t0_16.log, replace
set matsize 4000
...
foreach asvab in AR CS MK NO PC WK {
	ml model lf normal (asvab`asvab' = `Pvars97') /sigma if ~m_asvab`asvab' & firstObs
	ml max
	estimates store       reg`asvab'
	predict asvab`asvab'hatStructFirstObs, xb
	qui reg asvab`asvab' `Pvars97' if ~m_asvab`asvab' & agemo==2801
...
qui outreg2 [regAR regCS regMK regNO regPC regWK] using y97_asvab_noFvars_t0_16, excel replace bdec(4) sdec(4) ctitle(asvabs)
...
foreach asvab in AR CS MK NO PC WK {
	* Linear specification
	qui reg asvab`asvab' `Pvars97' `Fvars0'  `Svars00'  `Zvars00'  `Evars000' `risk' `work' if ~m_asvab`asvab' & agemo==2801
	...
	* Quadratic specification
	qui reg asvab`asvab' `Pvars97' `Fvars'  `Svars0'  `Zvars1'  `Evars00' `risk' `work' if ~m_asvab`asvab' & agemo==2801
	...
	* Preferred specification (currently listed in the NPdecompositions document)
	qui reg asvab`asvab' `Pvars97' `Fvars1'  `Svars1'  `Zvars1'  `Evars0'  `Ivars3' `risk' `work' if ~m_asvab`asvab' & agemo==2801
	...
	* More flexible specification
	qui reg asvab`asvab' `Pvars97' `Fvars15' `Svars15' `Zvars25' `Evars25' `Ivars3' `risk' `work' if ~m_asvab`asvab' & agemo==2801
	qui estat ic
	...
	* Most flexible specification
	qui reg asvab`asvab' `Pvars97' `Fvars2'  `Svars3'  `Zvars3'  `Evars3'  `Ivars3' `risk' `work' if ~m_asvab`asvab' & agemo==2801
	qui estat ic
...
foreach asvab in AR CS MK NO PC WK {
	...
	* Graphs of adjusted R2
	...
	graph export ${graphics_path}asvab`asvab'R2aFit97.eps, replace // can change to PNG if desired
	...
	* Graphs of residuals
	...
	graph export ${graphics_path}Dasvab`asvab'fit97.eps, replace // can change to PNG if desired
	...
	graph export ${graphics_path}Dasvab`asvab'compareDensity97.eps, replace // can change to PNG if desired
	...
	graph export ${graphics_path}Dasvab`asvab'scatter97.eps, replace // can change to PNG if desired
	...
	graph export ${graphics_path}Dasvab`asvab'scatterStruct97.eps, replace // can change to PNG if desired
	
	graph twoway ( scatter Dasvab`asvab'Lin   asvab`asvab' if agemo==2801 & ~m_asvab`asvab') ///
	, graphregion(color(white)) xtitle("Actual ASVAB") ytitle("Actual - Predicted ASVAB")
	graph export ${graphics_path}Dasvab`asvab'scatterLin97.eps, replace // can change to PNG if desired
	
	graph twoway ( scatter Dasvab`asvab'Quad   asvab`asvab' if agemo==2801 & ~m_asvab`asvab') ///
	, graphregion(color(white)) xtitle("Actual ASVAB") ytitle("Actual - Predicted ASVAB")
	graph export ${graphics_path}Dasvab`asvab'scatterQuad97.eps, replace // can change to PNG if desired
	
	graph twoway ( scatter Dasvab`asvab'         asvab`asvab' if agemo==2801 & ~m_asvab`asvab') ///
	, graphregion(color(white)) xtitle("Actual ASVAB") ytitle("Actual - Predicted ASVAB")
	graph export ${graphics_path}Dasvab`asvab'scatterFlex97.eps, replace // can change to PNG if desired
	
	graph twoway ( scatter Dasvab`asvab'MoreFlex asvab`asvab' if agemo==2801 & ~m_asvab`asvab') ///
	, graphregion(color(white)) xtitle("Actual ASVAB") ytitle("Actual - Predicted ASVAB")
	graph export ${graphics_path}Dasvab`asvab'scatterMoreFlex97.eps, replace // can change to PNG if desired
	
	graph twoway ( scatter Dasvab`asvab'MostFlex asvab`asvab' if agemo==2801 & ~m_asvab`asvab') ///
	, graphregion(color(white)) xtitle("Actual ASVAB") ytitle("Actual - Predicted ASVAB")
	graph export ${graphics_path}Dasvab`asvab'scatterMostFlex97.eps, replace // can change to PNG if desired
}
...
preserve
	...
	outsheet id year age agemo asvabAR asvabCS asvabMK asvabNO asvabPC asvabWK m_asvabAR m_asvabCS m_asvabMK m_asvabNO m_asvabPC m_asvabWK `Pvars97' `Fvars0' `Svars00' `Zvars00' `Evars000'        `risk' `work' using y97_asvab_fit_dataLinear.csv, comma replace nolabel
	outsheet id year age agemo asvabAR asvabCS asvabMK asvabNO asvabPC asvabWK m_asvabAR m_asvabCS m_asvabMK m_asvabNO m_asvabPC m_asvabWK `Pvars97' `Fvars1' `Svars1'  `Zvars1'  `Evars0' `Ivars3' `risk' `work' using y97_asvab_fit_dataChosen.csv, comma replace nolabel
	outsheet id year age agemo asvabAR asvabCS asvabMK asvabNO asvabPC asvabWK m_asvabAR m_asvabCS m_asvabMK m_asvabNO m_asvabPC m_asvabWK `Pvars97' `Fvars2' `Svars3'  `Zvars3'  `Evars3' `Ivars3' `risk' `work' using y97_asvab_fit_data.csv      , comma replace nolabel
restore

qui outreg2 [regARfit regCSfit regMKfit regNOfit regPCfit regWKfit] using y97_asvab_noFvars_t0_16_fit, excel replace bdec(4) sdec(4) ctitle(asvabs)
...
matrix parmAllse = [parmARse , parmCSse , parmMKse , parmNOse , parmPCse , parmWKse]
...
preserve
	...
	outsheet coefSigmaAll using y97_asvab_noFvars_t0_16_coef.csv, comma replace
restore

preserve
	keep seSigmaAll
	drop if seSigmaAll==.
	outsheet seSigmaAll using y97_asvab_noFvars_t0_16_se.csv, comma replace
restore
