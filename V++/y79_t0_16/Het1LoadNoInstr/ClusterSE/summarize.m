...
%   created with the STRUCT function.  See STRUCT for details.  Used
%   options are Names, Weights, and Detail. 
%      'Names' is a string vector of variable names for each column of X;
%   'Weights' is a matrix of sample weights (fweights in the Stata sense)
%   to compute weighted summary statistics. 'Detail' is either 'on' or
%   'off' to indicate whether or not detailed summary statistics should be
%   calculated.
%
%   Examples
...
%        sums = summarize([Acceleration Cylinders Displacement],options);
% 
%   See also MEAN, GETDESCRIPTIVESTATISTICS (MathWorks File Exchange).
% 
%   User notes
...
%   smallest and largest 5 observations
%   sum of weights
...
%   $Revision: 1.0.0.0 $  $Date: 2013/01/07 18:24:57 $
...
if ~isa(x,'double')
  error('optim:summarize:NonDoubleInput', ...
        'SUMMARIZE only accepts inputs of data type double.')
end
