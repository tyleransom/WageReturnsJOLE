clear all
tic
global argum d_beta d_alpha d_sigma d_ksi d_omega d_var d_mean nRestrA nRestrB rA rB GroupN
...
save arguments argum d_beta d_alpha d_sigma d_ksi d_omega d_mean d_var GroupN nRestrA nRestrB rA rB delta0 N1 nRestrMat rAB
clear 
load arguments d_beta d_alpha d_sigma d_omega d_ksi d_mean d_var GroupN argum nRestrA nRestrB rA rB delta0 N1 nRestrMat rAB
...
[aq1 aq2 aq3 aq4 aq5 aq6 aq7 aq8]=prok3(argum{yy,:});
cq1=cq1+aq1;
cq2=cq2+aq2;
cq3=cq3+aq3;
cq4=cq4+aq4;
...
[aqk dqk] = prok3_cpp_ofd(delta0); %checking the file for the optimization

gamma_f = delta0;
[gamma_f,fval,exitflag]=fminunc('prok3_cpp_ofd',delta0,options1);
...
% delta2=[d_beta; d_alpha; d_sigma; d_omega; d_ksi; gamma_f];
% argum2=argum;
...
% for yy=1:GroupN
	...
	% for i=1:N1(yy,1)
		...
		% [mq1 mq2 mq3 mq4 mq5 mq6]=prok3(argum2{yy,:});
		% eq=[mq2' mq3' mq4' mq5' mq6'];
		% fq = fq+eq';
		% if rB>0
...
% hessie1 = hessian(@(x) prok3_cpp_ofdd(x),gamma_f); %classic estimation of hessian
% hessie2a = jacobianest(@(x) prok3_cpp_ofddd(x),gamma_f); %estimation of the hessian from the gradients
...
%covMatr=hessie2\derek/hessie2; %sandwich formula
...
% save cov1b gamma_final covMatr sErrors
