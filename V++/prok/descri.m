...
present=numrows+1; %if the entry part of the model  
...
chng = maxival+1; %change for the marginal effects (continous variables, only in row 1), if 0 - average/100

auxvariable=chng+1; %id when auxiliary variable
...
numfac=average+1; %number of factors if LHS
...
parRestA=numAltRestA+1; %index: factor id for a restricted factor loading (dependent=1)
