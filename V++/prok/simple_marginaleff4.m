...
for mm=1:runnumber;
	...
	if regrattr(mm).changeregrcode(1,1)~=-1 
		...
		eval(['veff_' num2str(allregr) '= cat(3,veff_' num2str(allregr) ',hlf1);'])
	else
		...
		eval(['veff_' num2str(allregr) '= cat(3,veff_' num2str(allregr) ',hlf2);'])
	end
...
if mfx.C==1 
	mfx.var    = eval(['veff_' num2str(allregr)]);
else
	mfx.var    = eval(['veff_' num2str(allregr) '(:,:,end)']);
end
if mfx.C>1 
	for j=2:mfx.C
