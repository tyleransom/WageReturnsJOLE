...
set more off
set seed 32 // TROGDOR!!!
log using activity_tabulations32.log, replace
...
* EDIT: JA 2/26/2020: Eval at age 32 including education by that point
* Thus, no need for anything after 3112
drop if agemo>3112
...
* Note: we dropped all obs after 3112. As such, to determine
*  highest degree attained by age 32, look at the LASTOBS for
*  each person
*--------------------------------------------------------------------
...
foreach cohort in 1979 1997 {
	* N at 16 and 32, and average NT by age 32 (including those who dropped out)
	count if firstObs & cohortFlag==`cohort'
	local N_`cohort'_age16 = `r(N)'

	count if agemo==3112 & cohortFlag==`cohort'
	local N_`cohort'_age32 = `r(N)'

	sum numMonths if firstObs & cohortFlag==`cohort'
	local NT_`cohort'_age32 = `r(mean)'
}

* Demog and family
local v = 1
foreach var in white black hispanic foreignBorn hgcMoth hgcFath famInc femaleHeadHH14 {
	qui mean `var' [aw=weight] if agemo==3112 , over(cohortFlag)
	matrix m = e(b)
	foreach cohort in 1979 1997 {
    ...
    * forvalues x=1/4 {
		* qui mean `var' [aw=weight] if agemo==3112 & highDegAttain==`x', over(cohortFlag)
		* matrix m = e(b)
		* foreach cohort in 1979 1997 {
...
foreach y in 1979 1997 {
	qui sum afqt [aw=weight] if agemo==3112 & !m_afqt & cohortFlag==`y', d
	local med_`y'_all = `r(p50)'
	local  sd_`y'_all = `r(sd)'
	local   N_`y'_all = `r(N)'
	* forvalues x=1/4 {
		* qui sum afqt [aw=weight] if agemo==3112 & !m_afqt & highDegAttain==`x' & cohortFlag==`y', d
...
foreach y in 1979 1997 {
	local cc = `cc'+1
	qui bsqreg afqt if cohortFlag==`y' & agemo==3112 & ~m_afqt, reps(500) // weights not allowed
	matrix tester = e(V)
...
* foreach y in 1979 1997 {
	* local cc = `cc'+1
	* forvalues h = 1/4 {
		* qui bsqreg afqt if cohortFlag==`y' & highDegAttain==`h' & agemo==3112 & ~m_afqt, reps(500) // weights not allowed
		* matrix tester = e(V)
...
file open Tbackground using "${tbls_loc}Tbackground_32.tex", write replace
file write Tbackground "\begin{table}[ht]" _n 
* file write Tbackground "\caption{Demographic and AFQT performance by educational attainment at age 32}" _n 
...
file write Tbackground "\$N\$ at age 32              " " & " %6.0fc (`N_1979_age32')   " & " %6.0fc (`N_1997_age32')  " & & \\ "  _n 
file write Tbackground "%\$NT\$ at age 32              " " & " %6.0fc (`NT_1979_age32')   " & " %6.0fc (`NT_1997_age32')  " & & \\ "  _n 
...
file open Tdegree using "${tbls_loc}Tdegree_32.tex", write replace
file write Tdegree "\begin{table}[ht]" _n 
file write Tdegree "\caption{Schooling attainment and graduation probabilities at age 32}" _n 
...
file write Tdegree "~~\% HS Dropouts                     "  " & " %4.2f    (`mu_HSdropout_1979_3112') " & " %4.2f   (`mu_HSdropout_1997_3112') " & " %4.2f    (`mu_HSdropout_diff_3112')  " &    `mu_HSdropout_star_3112' \\ " _n
file write Tdegree "~~\% HS Graduates                    "  " & " %4.2f   (`mu_HSgraduate_1979_3112') " & " %4.2f  (`mu_HSgraduate_1997_3112') " & " %4.2f   (`mu_HSgraduate_diff_3112')  " &   `mu_HSgraduate_star_3112' \\ " _n
file write Tdegree "~~\% Some College                    "  " & " %4.2f    (`mu_BAdropout_1979_3112') " & " %4.2f   (`mu_BAdropout_1997_3112') " & " %4.2f    (`mu_BAdropout_diff_3112')  " &    `mu_BAdropout_star_3112' \\ " _n
file write Tdegree "~~\% College Graduates               "  " & " %4.2f   (`mu_BAgraduate_1979_3112') " & " %4.2f  (`mu_BAgraduate_1997_3112') " & " %4.2f   (`mu_BAgraduate_diff_3112')  " &   `mu_BAgraduate_star_3112' \\ " _n
...
file write Tdegree "%~~Pr(Grad HS)                        "  " & " %4.2f       (`mu_gradHS_1979_3112') " & " %4.2f      (`mu_gradHS_1997_3112') " & " %4.2f       (`mu_gradHS_diff_3112')  " &       `mu_gradHS_star_3112' \\ " _n
file write Tdegree "~~Pr(Start College)                  "  " & " %4.2f     (`mu_startCol_1979_3112') " & " %4.2f    (`mu_startCol_1997_3112') " & " %4.2f     (`mu_startCol_diff_3112')  " &     `mu_startCol_star_3112' \\ " _n
file write Tdegree "%~~Pr(Grad College)                   "  " & " %4.2f      (`mu_grad4yr_1979_3112') " & " %4.2f     (`mu_grad4yr_1997_3112') " & " %4.2f      (`mu_grad4yr_diff_3112')  " &      `mu_grad4yr_star_3112' \\ " _n
file write Tdegree "~~Pr(Grad College $\vert$ Start Col) "  " & " %4.2f  (`mu_grad4yrCond_1979_3112') " & " %4.2f (`mu_grad4yrCond_1997_3112') " & " %4.2f  (`mu_grad4yrCond_diff_3112')  " &  `mu_grad4yrCond_star_3112' \\ " _n
file write Tdegree "~~Time to College Degree (years)     "  " & " %4.2f   (`mu_yearsToCol_1979_3112') " & " %4.2f  (`mu_yearsToCol_1997_3112') " & " %4.2f   (`mu_yearsToCol_diff_3112')  " &   `mu_yearsToCol_star_3112' \\ " _n
file write Tdegree "%~~Time to College Degree (months)   "  " & " %4.2f  (`mu_monthsToCol_1979_3112') " & " %4.2f (`mu_monthsToCol_1997_3112') " & " %4.2f  (`mu_monthsToCol_diff_3112')  " &  `mu_monthsToCol_star_3112' \\ " _n
* file write Tdegree "\multicolumn{5}{l}{\emph{Number of Persons at age 32 (count):}} \\" _n 
* file write Tdegree "~~\$N\$ All Education Levels         "  " & " %6.0fc              (`N_1979_3112') " & " %6.0fc             (`N_1997_3112') " & "  " &   \\ " _n
...
*----------------------------------
* Table: Average end of panel experience
*----------------------------------
local v = 1
...
file open Texp using "${tbls_loc}Texp_32.tex", write replace
...
file write Texp "\footnotesize{Notes: All counts begin at age 16, thus the average individual in the NLSY79 had a total of 41 months of school after age 16. Statistics weighted by NLSY sampling weights. Significance reported at the 1\% (***), 5\% (**), and 10\% (*) levels.}" _n
...
foreach deg in 5 {
	...
	* drop flagger*
	foreach skill in anySchool workK12 workCollege workCollegeEarly workCollegeLate workPTonly workFTonly {
		* if `deg'<5 qui count if workFTonly==1 & agemo==3112 & highDegAttain==`deg' & cohortFlag==1979 & ~mi(lnWage)
		* else       qui count if workFTonly==1 & agemo==3112                        & cohortFlag==1979 & ~mi(lnWage)
		* local   en`skill'_1979_`deg' = `r(N)'
		
		gen `skill'tYears97 = `skill'tYears*(cohortFlag==1997)
		if `deg'<5 qui reg lnWage c.`skill'tYears c.`skill'tYears97 cohortFlag [aw=weight] if workFTonly==1 & agemo==3112 & highDegAttain==`deg'
		else       qui reg lnWage c.`skill'tYears c.`skill'tYears97 cohortFlag [aw=weight] if workFTonly==1 & agemo==3112
...
reg lnWage c.workCollegeEarlytYears c.workCollegeEarlytYears97 c.workCollegeLatetYears c.workCollegeLatetYears97 cohortFlag [aw=weight] if workFTonly==1 & agemo==3112
...
foreach deg in 1 2 3 4 {
	di "`deg'"
	foreach cohort in 1979 1997 {
		qui sum lnWage [aw=weight] if inlist(activity,4,14,24) & agemo==3112 & highDegAttain==`deg' & cohortFlag==`cohort'
	...
	qui mean lnWage [aw=weight] if inlist(activity,4,14,24) & agemo==3112 & highDegAttain==`deg', over(cohortFlag)
	matrix mm = r(table)
	foreach cohort in 1979 1997 {
	...
	qui sdtest lnWage if inlist(activity,4,14,24) & agemo==3112 & highDegAttain==`deg', by(cohortFlag) // weights not allowed
...
file open Twagepremia using "${tbls_loc}Twagepremia_32.tex", write replace
file write Twagepremia "\begin{table}[ht]" _n 
file write Twagepremia "\caption{Changes in wage premia for experience and educational attainment at age 32 for full-time workers}" _n 
...
foreach var in empPct incPerCapita numBAperCapita posBA tuitionFlagship {
    foreach x in 1612 2212 2512 2812 3112 {
...
file open Tlocal using "${tbls_loc}Tlocal_32.tex", write replace
