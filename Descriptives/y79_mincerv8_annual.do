...
capture log close
global runname y79_mincerv8_annual
...
use if cohortFlag==1979 & ~female & inlist(activity,2,3,4,12,13,14,22,23,24) using ${secretdir}/yCombined_scaled_t0_16.dta, clear
drop if inlist(birthYear,1957,1958)
bys id age: egen modalActivity = mode(activity), maxmode
replace activity=modalActivity
drop modalActivity
keep if monthAlt==12
drop lnWage
...
* HLT + Background
ml model lf normal (lnWage =  `Pvars79' `Mvars' `Svars3'    c.potExpt##c.potExpt##c.potExpt c.anySchoolt#c.potExpt `risk' `work') /sigma2 /sigma3 /sigma4 , vce(cluster id)
ml max
global tmpvars anySchoolt potExpt `risk'
...
ml model lf normal (lnWage = `Pvars79' `Mvars' `Svars3'    `Zvars3I' `Evars3I'                                     `risk' `work') /sigma2 /sigma3 /sigma4 , vce(cluster id)
ml max
