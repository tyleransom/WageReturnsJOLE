...
set maxvar 32000
version 11.0
capture cd "/afs/econ.duke.edu/data/vjh3/WageReturns/Data/BEA"
capture log close
set more off
log using BEA_create_MSA.log, replace
...
foreach state in `states' {
	* Get per-captia income and total employment by county
	...
	keep FIPS Linecode AreaName yr* // this keeps the FIPS code, the Linecode and the year
	tostring yr*, replace
	reshape long yr, i(FIPS Linecode) j(year)
	ren yr value
...
foreach x of numlist 1/9 {
	bys FIPS (year): gen growthInd`x' = ind`x'Share[_n-1]*((ind`x'Emp[_n]-ind`x'Emp[_n-1])/ind`x'Emp[_n-1])
}
