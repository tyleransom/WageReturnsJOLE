...
set more off
capture log close
log using "/afs/econ.duke.edu/data/vjh3/WageReturns/Data/y97/RawData/AFQT_MATCHING_WK/AFQT_MATCHING_WK_with_weights.log", replace
...
capture cd "/afs/econ.duke.edu/data/vjh3/WageReturns/Data/y97/RawData/AFQT_MATCHING_WK"
...
ren v1 pid
ren v2 male
gen asvabWK=wk if wk!=0 // pc=paragraph comprehension, no=numerical comprehension 
...
kdensity asvabWK if sample==0&age==16, addplot(kdensity asvabWK if sample==1&age==16, lpattern(_)) title(Figure 1: AFQT Scores at Age 16) ///
...
bys sample pasvabWK: egen tot_score=sum(asvabWK*weight) if age==16 	// Mean age=16 raw score for each percentile //
bys sample pasvabWK: gen mean=tot_score/pop if age==16
drop tot_score pop
egen tag=tag(sample pasvabWK mean) if age==16 			// We need only 1 obs per sample and percentile //
keep if tag==1
...
merge sample pasvabWK using `agestd_afqt' // Merge into each percentile the age=16 corresponding score //
drop _merge
keep sample pid male pasvabWK asvabWK_std asvabWK weight age
sort sample pid
// The final data contains "asvabWK_std": which is the comparable score across the 2 NLSY samples //
...
*drop sample age asvabWK pasvabWK // drop the other variables since I don't use them in my research //
preserve
	use `afqt97', clear
	keep pid male
	ren pid ID
