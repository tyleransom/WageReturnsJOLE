...
set more off
capture log close
log using "/afs/econ.duke.edu/data/vjh3/WageReturns/Data/y97/RawData/AFQT_MATCHING_CS/AFQT_MATCHING_CS_with_weights.log", replace
...
capture cd "/afs/econ.duke.edu/data/vjh3/WageReturns/Data/y97/RawData/AFQT_MATCHING_CS"
...
ren v1 pid
ren v2 male
gen asvabCS=cs if cs!=0 // pc=paragraph comprehension, no=numerical comprehension 
keep pid male asvabCS
sort pid
save `afqt97', replace
...
gen asvabCS = cs if cs!=0
drop if asvabCS==.
label var asvabCS "Arithmetic Reasoning subset of ASVAB score"
...
// Drop those with missing AFQT CS scores
drop if asvabCS==.
...
kdensity asvabCS if sample==0&age==16, addplot(kdensity asvabCS if sample==1&age==16, lpattern(_)) title(Figure 1: AFQT Scores at Age 16) ///
...
bysort sample age: egen r=rank(asvabCS), u
gen pasvabCS=.
...
*SAMPLE==0
qui replace pasvabCS=round(r/sumWgts) if age==15 & sample==0
qui replace pasvabCS=round(r/sumWgts) if age==16 & sample==0
qui replace pasvabCS=round(r/sumWgts) if age==17 & sample==0
qui replace pasvabCS=round(r/sumWgts) if age==18 & sample==0
qui replace pasvabCS=round(r/sumWgts) if age==19 & sample==0
qui replace pasvabCS=round(r/sumWgts) if age==20 & sample==0
qui replace pasvabCS=round(r/sumWgts) if age==21 & sample==0
qui replace pasvabCS=round(r/sumWgts) if age==22 & sample==0

*SAMPLE 1
qui replace pasvabCS=round(r/sumWgts) if age==12 & sample==1
qui replace pasvabCS=round(r/sumWgts) if age==13 & sample==1
qui replace pasvabCS=round(r/sumWgts) if age==14 & sample==1
qui replace pasvabCS=round(r/sumWgts) if age==15 & sample==1
qui replace pasvabCS=round(r/sumWgts) if age==16 & sample==1
qui replace pasvabCS=round(r/sumWgts) if age==17 & sample==1
...
bys sample pasvabCS: egen pop=sum(weight) if age==16
bys sample pasvabCS: egen tot_score=sum(asvabCS*weight) if age==16 	// Mean age=16 raw score for each percentile //
bys sample pasvabCS: gen mean=tot_score/pop if age==16
drop tot_score pop
egen tag=tag(sample pasvabCS mean) if age==16 			// We need only 1 obs per sample and percentile //
keep if tag==1
ren mean asvabCS_std
keep sample asvabCS_std pasvabCS asvabCS
sort sample pasvabCS
save `agestd_afqt', replace

use `nlsy_agestd', clear
merge sample pasvabCS using `agestd_afqt' // Merge into each percentile the age=16 corresponding score //
drop _merge
keep sample pid male pasvabCS asvabCS_std asvabCS weight age
sort sample pid
// The final data contains "asvabCS_std": which is the comparable score across the 2 NLSY samples //
...
*drop sample age asvabCS pasvabCS // drop the other variables since I don't use them in my research //
preserve
	use `afqt97', clear
	keep pid male
	ren pid ID
...
preserve
	...
	zscore asvabCS_std
	drop asvabCS_std
	ren z_asvabCS_std asvabCS_std
	keep if male==1
	sort ID
	outsheet ID asvabCS_std using altonjiAFQT.csv, comma nol replace
restore
save afqt_adjusted_final, replace 
save `finished_product', replace

tabstat asvabCS_std, by(sample) stats(n min max mean median)
...
**bys sample age: sum asvabCS_std [fw=weight], d
