...
set more off
capture log close
log using "/afs/econ.duke.edu/data/vjh3/WageReturns/Data/y97/RawData/AFQT_MATCHING_NO/AFQT_MATCHING_NO_with_weights.log", replace
...
capture cd "/afs/econ.duke.edu/data/vjh3/WageReturns/Data/y97/RawData/AFQT_MATCHING_NO"
...
ren v1 pid
ren v2 male
gen asvabNO=no if no!=0 // pc=paragraph comprehension, no=numerical comprehension 
keep pid male asvabNO
sort pid
save `afqt97', replace
...
gen asvabNO = no if no!=0
drop if asvabNO==.
label var asvabNO "Arithmetic Reasoning subset of ASVAB score"
...
// Drop those with missing AFQT NO scores
drop if asvabNO==.
...
kdensity asvabNO if sample==0&age==16, addplot(kdensity asvabNO if sample==1&age==16, lpattern(_)) title(Figure 1: AFQT Scores at Age 16) ///
...
bysort sample age: egen r=rank(asvabNO), u
gen pasvabNO=.
...
*SAMPLE==0
qui replace pasvabNO=round(r/sumWgts) if age==15 & sample==0
qui replace pasvabNO=round(r/sumWgts) if age==16 & sample==0
qui replace pasvabNO=round(r/sumWgts) if age==17 & sample==0
qui replace pasvabNO=round(r/sumWgts) if age==18 & sample==0
qui replace pasvabNO=round(r/sumWgts) if age==19 & sample==0
qui replace pasvabNO=round(r/sumWgts) if age==20 & sample==0
qui replace pasvabNO=round(r/sumWgts) if age==21 & sample==0
qui replace pasvabNO=round(r/sumWgts) if age==22 & sample==0

*SAMPLE 1
qui replace pasvabNO=round(r/sumWgts) if age==12 & sample==1
qui replace pasvabNO=round(r/sumWgts) if age==13 & sample==1
qui replace pasvabNO=round(r/sumWgts) if age==14 & sample==1
qui replace pasvabNO=round(r/sumWgts) if age==15 & sample==1
qui replace pasvabNO=round(r/sumWgts) if age==16 & sample==1
qui replace pasvabNO=round(r/sumWgts) if age==17 & sample==1
...
bys sample pasvabNO: egen pop=sum(weight) if age==16
bys sample pasvabNO: egen tot_score=sum(asvabNO*weight) if age==16 	// Mean age=16 raw score for each percentile //
bys sample pasvabNO: gen mean=tot_score/pop if age==16
drop tot_score pop
egen tag=tag(sample pasvabNO mean) if age==16 			// We need only 1 obs per sample and percentile //
keep if tag==1
ren mean asvabNO_std
keep sample asvabNO_std pasvabNO asvabNO
sort sample pasvabNO
save `agestd_afqt', replace

use `nlsy_agestd', clear
merge sample pasvabNO using `agestd_afqt' // Merge into each percentile the age=16 corresponding score //
drop _merge
keep sample pid male pasvabNO asvabNO_std asvabNO weight age
sort sample pid
// The final data contains "asvabNO_std": which is the comparable score across the 2 NLSY samples //
...
*drop sample age asvabNO pasvabNO // drop the other variables since I don't use them in my research //
preserve
	use `afqt97', clear
	keep pid male
	ren pid ID
...
preserve
	...
	zscore asvabNO_std
	drop asvabNO_std
	ren z_asvabNO_std asvabNO_std
	keep if male==1
	sort ID
	outsheet ID asvabNO_std using altonjiAFQT.csv, comma nol replace
restore
save afqt_adjusted_final, replace 
save `finished_product', replace

tabstat asvabNO_std, by(sample) stats(n min max mean median)
...
**bys sample age: sum asvabNO_std [fw=weight], d
